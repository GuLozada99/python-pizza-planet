...
def __create_items(items: list, controller: Type[BaseController]):
    created_items = []
...
def test_create(app, ingredients, beverages, size, client_data):
    ...
    client_data = {f'client_{key}': value for key, value in client_data.items()}
    ...
    client_dni = order.pop('client_dni', None)
    ...
    pytest.assume(client_dni == created_order['client']['dni'])
...
def test_get_all(app, ingredients, beverages, sizes, client_data):
    ...
    created_orders = []
    for _ in range(5):
        order = __order(shuffle_list(created_ingredients)[:3], shuffle_list(created_beverages)[:3],
...
def test_consecutive_create(app, ingredients, beverages, size, client_data):
    created_size, created_ingredients, created_beverages = __create_sizes_beverages_and_ingredients(
            ingredients, beverages, [size]
    )
    client_data = {f'client_{key}': value for key, value in client_data.items()}
    ...
    second_order = __order(created_ingredients, created_beverages,
                           created_size, {'client_dni': client_dni})
    second_created_order, error = OrderController.create(second_order)

    pytest.assume(error is None)
    pytest.assume(second_created_order['_id'])
    pytest.assume(client_dni == second_created_order['client']['dni'])
